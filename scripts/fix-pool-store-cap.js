...
db.ready
  ...
  `,
    ...
    console.log(
      ...
      toRemove.map(
        (e) =>
          `slot ${e.slot_index} [${e.store_id}] ${e.product_name_snapshot}`,
      ),
    );
    ...
    for (const e of toRemove) {
      ...
      console.log(
        `Removed slot ${e.slot_index}: [${e.store_id}] ${e.product_name_snapshot}`,
      );
    }
    ...
    for (const r of remaining) {
      remainingStoreCounts[r.store_id] =
        (remainingStoreCounts[r.store_id] || 0) + 1;
    }
  ...
  `,
    ...
    `,
      ...
      if (!materialized) {
        console.log(
          `  Skipping (won't materialize): [${deal.store_id}] ${deal.product_name}`,
        );
        await db
          .prepare(
            "DELETE FROM daily_deal_pool_entries WHERE pool_date = ? AND slot_index = ?",
      ...
      remainingStoreCounts[deal.store_id] =
        (remainingStoreCounts[deal.store_id] || 0) + 1;
      console.log(
