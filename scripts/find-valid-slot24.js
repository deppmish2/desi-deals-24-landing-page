...
db.ready
  .then(async () => {
    ...
    const { fetchActiveDealRows } = (() => {
      try {
        return require("../server/services/daily-deals-pool");
      } catch (e) {
        return {};
      }
    })();
  ...
  `,
    ...
    if (!resolveBaseProduct) {
      ...
      candidates.forEach((c) =>
        console.log(`[${c.store_id}] ${c.product_name} | €${c.sale_price}`),
      );
    } else {
      ...
      valid
        ...
        .forEach((c) =>
          console.log(
            `[${c.store_id}] [${c.product_category}] ${c.product_name} | €${c.sale_price} | ${c.product_url}`,
          ),
        );
    }
    process.exit(0);
