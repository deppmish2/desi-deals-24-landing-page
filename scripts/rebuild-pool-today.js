...
const REPEAT_WINDOW_DAYS = Number(
...
db.ready
  ...
  `,
    ...
    let pool = await getDailyDealsPool(db, {
      poolDate,
      limit: 50,
      allowGenerate: false,
    });
    ...
    for (const r of pool.rows)
      storeCounts[r.store_id] = (storeCounts[r.store_id] || 0) + 1;
    ...
    const usedSigs = new Set(
      (
        await db
          .prepare(
            "SELECT product_signature FROM daily_deal_pool_entries WHERE pool_date = ?",
          )
          .all(poolDate)
    ...
    const usedDealIds = new Set(
      (
        await db
          .prepare(
            "SELECT deal_id FROM daily_deal_pool_entries WHERE pool_date = ? AND deal_id IS NOT NULL",
          )
          .all(poolDate)
      ).map((r) => r.deal_id),
    );

    // Collect deal IDs used in the repeat window (past N days, excluding today) to avoid cross-day repeats
    const recentWindowStart = new Date(poolDate);
    recentWindowStart.setUTCDate(
      recentWindowStart.getUTCDate() - (REPEAT_WINDOW_DAYS - 1),
    );
    const recentWindowStartStr = recentWindowStart.toISOString().slice(0, 10);
    const recentDealIds = new Set(
      (
        await db
          .prepare(
            "SELECT deal_id FROM daily_deal_pool_entries WHERE pool_date >= ? AND pool_date < ? AND deal_id IS NOT NULL",
          )
          .all(recentWindowStartStr, poolDate)
      ).map((r) => r.deal_id),
    );
    console.log(
      `Repeat-window exclusions (${recentWindowStartStr} to ${poolDate}):`,
      recentDealIds.size,
    );

    // 3. Get all eligible active deals not in pool or recent window, ordered by last_pool_used_at
  ...
  `,
    ...
    `,
      ...
      const check = await getDailyDealsPool(db, {
        poolDate,
        limit: 50,
        allowGenerate: false,
      });
    ...
    // 4. Final check
    const final = await getDailyDealsPool(db, {
      poolDate,
      limit: 24,
      allowGenerate: false,
    });
    ...
    for (const r of final.rows)
      finalStoreCounts[r.store_id] = (finalStoreCounts[r.store_id] || 0) + 1;
