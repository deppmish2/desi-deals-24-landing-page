...
## [2026-05-08] auto-update | Admin canonical category change + cascade service + search word-boundary fix

Pages touched: backend.md, frontend.md
Sources: server/services/category-cascade.js, server/routes/admin-dashboard.js, server/routes/catalog.js, server/routes/store-products.js, client/src/components/AdminProductEditor.jsx, client/src/components/CartButton.jsx, client/src/components/ProductCard.jsx, client/src/pages/CatalogPage.jsx, client/src/pages/DealsPage.jsx, client/src/utils/api.js

**Changes recorded:**

- `backend.md`: Added `POST /api/v1/admin-dashboard/canonical/:id/change-category` route to route table. Added `GET /api/v1/admin-dashboard/mapped-products`. Added "Category cascade service" section documenting `cascadeCategoryChange(db, canonicalId, newCategory)` algorithm: canonical update → load mapped store products → per-product matchesCanonical re-test → batch DELETE/INSERT/queue. Documented `VALID_CATEGORIES` (18 values, now includes "Snacks & Namkeen"). Updated catalog route row: noted word-boundary LIKE (`' ' || lower(col) || ' ' LIKE '% term %'`) prevents synonym false positives (e.g. `aval` matching "Athavale" brand name). Added "Store-products serialization" section: `primary_brand` exposed via `json_extract(cp.brand_slots, '$[0][0]')` LEFT JOIN.
- `frontend.md`: Added `changeCanonicalCategory(id, category)` and `updateCanonical(id, data)` to admin API functions list. Rewrote `ProductCard` section: `isAdmin` prop from CatalogPage, opens `AdminProductEditor` overlay passing `initialCategory`. Added new `AdminProductEditor` section: 18-category select, amber remap warning, cascade-first save handler, `dd24-canonical-updated` event, 2s auto-close. Added new `CartButton` section: brand detection priority (primary_brand → sliding-window name scan → anyBrand), enriched cart item shape.
- **Bug fixes documented:** Catalog search `LIKE '%aval%'` was matching "Athavale Corn Chakali Sticks" when searching "poha" (aval = Tamil synonym for flattened rice). Fixed by padding both column and term with spaces to enforce word boundaries.
