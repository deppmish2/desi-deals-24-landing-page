...
test("resolveBaseProduct: branded paneer product resolves to base_key paneer", () => {
...
test("resolveBaseProduct: MDH Karahi Paneer Masala resolves to paneer masala, not paneer", () => {
  const result = resolveBaseProduct("MDH Karahi Paneer Masala");
  assert.ok(result, "expected a match — row 1007 (Paneer Masala) may be missing from the CSV");
  assert.equal(result.base_key, "paneer masala",
    "'MDH Karahi Paneer Masala' must resolve to base_key 'paneer masala' — " +
    "check row 1007 in 'data/Most Popular Indian Groceries - indian_grocery_1000_items.csv'");
});
