...
async function recommendForList(
    ...
    for (const item of items) {
      ...
      if (hasMassVolumePricingTarget && requestedBaseMeta) {
        ...
        if (!selectedStrict) {
          ...
          if (nearestOpt) {
            const snapDeal = nearestOpt.deal;
            selectedStrict = {
              base_product: requestedBaseMeta.base_product,
              brand: detectBrandForBase(snapDeal.product_name, requestedBaseMeta.base_key) || extractLikelyBrand(snapDeal.product_name) || null,
              brand_key: null,
              representative_deal: snapDeal,
              combination: [{ product_name: snapDeal.product_name, product_url: snapDeal.product_url, sale_price: snapDeal.sale_price, weight_value: snapDeal.weight_value, weight_unit: snapDeal.weight_unit, count: 1 }],
              packs_needed: 1,
              total_price: Number(snapDeal.sale_price),
              requested_brand_matched: requestedBrandCandidates.length === 0 || requestedBrandCandidates.some((b) => isBrandMatch(b, snapDeal.product_name)),
            };
          }
        }
        if (!selectedStrict) {
  ...
  const hasAnyAutoCartStore = stores.some((s) =>
    ["shopify", "woocommerce"].includes(String(s.platform || "").toLowerCase()),
  );
  ...
  return {
    ...
    winner,
    runner_up: runnerUp,
    stores: ranked.filter((row) => row.auto_cart_supported),
    ...
    reason: winner
      ? null
      : hasAnyAutoCartStore
        ? "no_store_with_any_match_and_auto_cart"
        : "no_store_with_any_match",
    summary: {
