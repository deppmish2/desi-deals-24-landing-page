...
async function ensureDailyDealsPool(db, options = {}) {
  ...
  if (entries.length > 0) {
    // Verify that enough pool entries still resolve to active, eligible deals.
    // A post-crawl run marks stale deals is_active=0, which would leave the pool
    // pointing at dead entries and returning "No deals found" to users.
    const activeRow = await db
      ...
      WHERE e.pool_date = ?
        AND d.is_active = 1
        AND lower(coalesce(d.availability, '')) = 'in_stock'
        AND d.discount_percent >= ${DAILY_POOL_MIN_DISCOUNT}
    `,
      )
      .get(poolDate);
    const activeCount = Number(activeRow?.cnt || 0);

    if (activeCount >= DAILY_POOL_LIMIT) {
      return { entries, poolDate, requestedPoolDate: poolDate };
    }

    console.log(
      `[daily-pool] Pool for ${poolDate} has only ${activeCount}/${entries.length} active entries — deleting and regenerating.`,
    );
    await db
