...
async function validateCanonicalMatch(query, canonicalId, quantity = null, quantityUnit = null) {
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const list = await getOwnedList(listId, req.user.id);
...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    const updated = await getOwnedList(list.id, req.user.id);
...
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    await db.prepare("DELETE FROM shopping_lists WHERE id = ? AND user_id = ?").run(
      list.id,
      req.user.id,
    );
    res.json({ ok: true });
