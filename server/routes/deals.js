...
router.get("/", async (req, res, next) => {
  ...
  try {
    ...
    if (!pool) {
      ...
      // Only cache when the pool is actually for today — never cache a stale
      // fallback (stale_while_missing_pool / stale_while_crawl), otherwise the
      // first early-morning request would lock in yesterday's deals for 5 min
      // even after the crawl+rebuild populates today's pool.
      const isActuallyToday =
        isToday &&
        pool.rows.length > 0 &&
        !pool.meta?.stale_while_missing_pool &&
        !pool.meta?.stale_while_crawl;
      if (isActuallyToday) {
